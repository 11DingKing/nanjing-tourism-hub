...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "shelter reservation operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("shelter reservation operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "shelter reservation", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
