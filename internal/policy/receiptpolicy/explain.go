...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "field receipt operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("field receipt operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "field receipt", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
