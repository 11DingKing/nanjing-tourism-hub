...
func Evaluate(ctx context.Context, input Input) Decision {
	...
	if !roleAllowed(input.ActorRole) {
		return deny("role_forbidden", fmt.Sprintf("role %s cannot manage field receipt", input.ActorRole))
	}
	...
	if input.CurrentState != input.TargetState && !transitionAllowed(input.CurrentState, input.TargetState) {
		return deny("transition_rejected", fmt.Sprintf("cannot move %s from %s to %s", "field receipt", input.CurrentState, input.TargetState))
	}
	...
	return Decision{Allowed: true, Code: "allowed", Obligations: []string{"verify_task_scope", "retain_evidence", "record_review"}}
}
...
func RequireAllowed(decision Decision) error {
	...
	return fmt.Errorf("field receipt policy %s: %s", decision.Code, decision.Reason)
}
