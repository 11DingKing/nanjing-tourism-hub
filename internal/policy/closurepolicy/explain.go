...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "incident closure operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("incident closure operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "incident closure", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
