...
func Evaluate(ctx context.Context, input Input) Decision {
	...
	if !roleAllowed(input.ActorRole) {
		return deny("role_forbidden", fmt.Sprintf("role %s cannot manage incident closure", input.ActorRole))
	}
	...
	if input.CurrentState != input.TargetState && !transitionAllowed(input.CurrentState, input.TargetState) {
		return deny("transition_rejected", fmt.Sprintf("cannot move %s from %s to %s", "incident closure", input.CurrentState, input.TargetState))
	}
	...
	return Decision{Allowed: true, Code: "allowed", Obligations: []string{"settle_open_tasks", "settle_receipts", "retain_audit_history"}}
}
...
func RequireAllowed(decision Decision) error {
	...
	return fmt.Errorf("incident closure policy %s: %s", decision.Code, decision.Reason)
}
