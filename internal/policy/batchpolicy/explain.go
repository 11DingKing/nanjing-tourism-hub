...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "batch assignment operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("batch assignment operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "batch assignment", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
