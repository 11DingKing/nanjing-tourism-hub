...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "regional query operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("regional query operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "regional query", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
