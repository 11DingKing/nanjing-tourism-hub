...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "durable job operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("durable job operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "durable job", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
