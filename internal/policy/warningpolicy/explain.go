...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "warning operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("warning operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "warning", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
