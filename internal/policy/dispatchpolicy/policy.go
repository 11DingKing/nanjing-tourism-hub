...
func Evaluate(ctx context.Context, input Input) Decision {
	...
	if !roleAllowed(input.ActorRole) {
		return deny("role_forbidden", fmt.Sprintf("role %s cannot manage team dispatch", input.ActorRole))
	}
	...
	if input.CurrentState != input.TargetState && !transitionAllowed(input.CurrentState, input.TargetState) {
		return deny("transition_rejected", fmt.Sprintf("cannot move %s from %s to %s", "team dispatch", input.CurrentState, input.TargetState))
	}
	...
	return Decision{Allowed: true, Code: "allowed", Obligations: []string{"lock_team_version", "bind_task_owner", "emit_dispatch_event"}}
}
...
func RequireAllowed(decision Decision) error {
	...
	return fmt.Errorf("team dispatch policy %s: %s", decision.Code, decision.Reason)
}
