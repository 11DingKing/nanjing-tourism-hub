...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "team dispatch operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("team dispatch operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "team dispatch", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
