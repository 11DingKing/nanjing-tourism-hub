...
func Evaluate(ctx context.Context, input Input) Decision {
	...
	if !roleAllowed(input.ActorRole) {
		return deny("role_forbidden", fmt.Sprintf("role %s cannot manage supply allocation", input.ActorRole))
	}
	...
	if input.CurrentState != input.TargetState && !transitionAllowed(input.CurrentState, input.TargetState) {
		return deny("transition_rejected", fmt.Sprintf("cannot move %s from %s to %s", "supply allocation", input.CurrentState, input.TargetState))
	}
	...
	return Decision{Allowed: true, Code: "allowed", Obligations: []string{"lock_lot_version", "preserve_available_quantity", "record_audit"}}
}
...
func RequireAllowed(decision Decision) error {
	...
	return fmt.Errorf("supply allocation policy %s: %s", decision.Code, decision.Reason)
}
