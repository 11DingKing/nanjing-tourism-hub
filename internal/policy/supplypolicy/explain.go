...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "supply allocation operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("supply allocation operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "supply allocation", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
