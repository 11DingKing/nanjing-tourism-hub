...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "response operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("response operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "response", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
