...
func Evaluate(ctx context.Context, input Input) Decision {
	...
	if !roleAllowed(input.ActorRole) {
		return deny("role_forbidden", fmt.Sprintf("role %s cannot manage district task", input.ActorRole))
	}
	...
	if input.CurrentState != input.TargetState && !transitionAllowed(input.CurrentState, input.TargetState) {
		return deny("transition_rejected", fmt.Sprintf("cannot move %s from %s to %s", "district task", input.CurrentState, input.TargetState))
	}
	...
	return Decision{Allowed: true, Code: "allowed", Obligations: []string{"enforce_state_machine", "preserve_assignee", "emit_task_event"}}
}
...
func RequireAllowed(decision Decision) error {
	...
	return fmt.Errorf("district task policy %s: %s", decision.Code, decision.Reason)
}
