...
func Explain(decision Decision, metadata map[string]string) Explanation {
	...
	summary := "operator session operation is allowed"
	if !decision.Allowed {
		summary = fmt.Sprintf("operator session operation rejected: %s", decision.Reason)
	}
	return Explanation{Subject: "operator session", Allowed: decision.Allowed, Summary: summary, Obligations: obligations, Metadata: copyMetadata}
}
